...
double calculateRadiationLevel(int sensorId, time_t lastUpdate) {
    double baseRadiation = Config::getModerateRadiationLevel(config) + 1.5;
    double variation = 0.2;  
    double timeFactor = 0.0001;  

    double geoFactor = sin((sensorId % 360) * M_PI / 180.0);
    ...
    return mymax(1.5, radiationLevel);  
}
...
void updateSensorRadiation(std::vector<Sensor>& sensors, const std::string& token) {
    CURL* curl = curl_easy_init();
    ...
    for (auto& sensor : sensors) {
        double radiationLevel = calculateRadiationLevel(sensor.id, time(nullptr));
        radiationLevel = roundToTwoDecimalPlaces(radiationLevel);

        std::cout << "Sensor ID: " << sensor.id << ", Radiation: " << std::fixed << std::setprecision(2) << radiationLevel << " mSv" << std::endl;

        json data;
        data["sensor"] = sensor.id;
        data["radiation_level"] = radiationLevel;
        data["alert_triggered"] = (radiationLevel >= Config::getHighRadiationLevel(config));

        std::string data_str = data.dump();
        std::cout << "Radiation data JSON: " << data_str << std::endl;
...
int main() {
    ...
    while (true) {
        if (!token.empty()) {
            ...
            updateSensorRadiation(sensors,  token);
        }
